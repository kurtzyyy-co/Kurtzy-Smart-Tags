...
try {
  ...
  console.error("Error loading firebase-applet-config.json on server:", e);
}
...
if (isFirebaseConfigured) {
  try {
    ...
    console.log(`Firebase initialized successfully on server backend! Project: ${firebaseConfig.projectId}, Database ID: ${firestoreDatabaseId || "(default)"}`);
  } catch (err) {
