...
import SkeletonScreen from "./components/TagSkeletonScreen";
...
export default function App() {
    ...
    const fetchMetrics = async () => {
      try {
        ...
        if (res.ok && active) {
          ...
          if (data && data.rateLimiter) {
            setCustomMaxRequests(data.rateLimiter.maxRequests);
          }
        }
      } catch (err) {
        console.error("Failed to fetch system operations metrics:", err);
      }
    };
  ...
  const handleUpdateSystemConfig = async (newAlgo?: string, rlEnabled?: boolean, limit?: number, nodeStatuses?: Record<string, string>) => {
    try {
      ...
      if (res.ok) {
        ...
        if (metricsRes.ok) {
          setSystemMetrics(await metricsRes.ok ? await metricsRes.json() : null);
        }
      }
    } catch (err) {
  ...
  // Check PIN to enter Admin mode securely via backend operations server
